...
import traceback
import redis
from interface_platform.settings import redis_port

# 初始化框架工程中的全局变量，存储在测试数据中的唯一值数据
# 框架工程中若要使用字典中的任意一个变量，则每次使用后，均需要将字典中的value值进行加1操作。

pool = redis.ConnectionPool(host='127.0.0.1', port=redis_port, db=2, decode_responses=True)
...
def get_unique_number_value(unique_number):
    ...
    try:
        ...
        else:  # 初始化unique_number变量到redis库第三个库，因为前两个库被celery用了
            data = 777
